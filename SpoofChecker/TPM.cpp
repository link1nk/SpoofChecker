...
std::string TPM::BytesToString(const std::vector<BYTE>& bytes)
{
	std::string result;
...
std::string TPM::GetKeyHash(const std::vector<BYTE>& input, ALG_ID algo)
{
...
void TPM::ShowTPMInfo()
{
	UTILS::PrintFixedBox("TPM");

	std::vector<BYTE> ekData = GetEK();
	if (ekData.empty())
	{
		printf("Failed to retrieve EK\n");
	}

	std::cout << dye::light_purple("MD5") << ":     " << dye::green(GetKeyHash(ekData, CALG_MD5).c_str()) << std::endl;
	std::cout << dye::light_purple("SHA1") << ":    " << dye::green(GetKeyHash(ekData, CALG_SHA1).c_str()) << std::endl;
	std::cout << dye::light_purple("SHA256") << ":  " << dye::green(GetKeyHash(ekData, CALG_SHA_256).c_str()) << std::endl;
